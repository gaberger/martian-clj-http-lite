...
(defn bootstrap-openapi [url & [opts]]
  (let [{:keys [http-opts]} opts
        definition (-> (http/get url http-opts) :body (json/parse-string keyword)) ;; clj-http-lite does not support {:as :json} body conversion (yet) so we do it right here
